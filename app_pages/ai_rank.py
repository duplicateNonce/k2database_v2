...
def render_ai_rank_page():
    ...
    hours = st.selectbox(
        "\u7edf\u8ba1\u65f6\u957f (\u5c0f\u65f6)", [4, 12, 24, 72, 168], index=2, format_func=lambda x: f"\u6700\u8fd1 {x}h"
    )
    top_n = st.number_input("\u8ba1\u5165\u6b21\u6570\u7684\u524d N \u540d", min_value=1, value=40, step=1)
        ...
        for _, row in top_df.iterrows():
            ...
            with st.spinner(f"{sym} \u5206\u6790\u4e2d..."):
                try:
                    ...
                    st.markdown(f"**{sym}**: {answer}")
                except Exception as exc:
                    st.error(f"{sym} \u67e5\u8be2\u5931\u8d25: {exc}")

        st.subheader("\u81ea\u5b9a\u4e49\u6807\u7684 AI \u5206\u6790")
        symbol = st.selectbox("\u9009\u62e9\u4ee3\u5e01", stats["\u4ee3\u5e01\u540d\u5b57"], key="ai_rank_symbol")
